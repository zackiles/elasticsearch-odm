...
function QueryPromise(query, func){
  ...
  self.sort = function(value){
    Query.addSorts(self.q, value);
    return self;
  };

  // If then is never called, then execute anyways.
  // This mimics
  setTimeout(function(){
    self.exec();
  }, 1000);
...
module.exports = {
  ...
  find: function(match, queryOptions) {
    ...
    return new QueryPromise(query, func);
  },

  search: function(queryOptions) {
    return this.find(null, queryOptions);
  },
  ...
  findOne: function(match, fields) {
    ...
    var query = Query.parseRequest(self.db.index, self.model.type, queryOptions);
