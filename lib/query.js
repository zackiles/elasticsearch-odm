//'use strict';
// can't user strict mode, elasticsearch queries ssometimes contains
...
function addExistenceFilters(req, missingFilters, existsFilters){
  ...
  var getPreviousFilters = function(){
    var andFilters;
    if(boolFilter){
      andFilters = dot.pick('body.query.filtered.query.constant_socre.filter.and', req);
    }else{
      andFilters = dot.pick('body.query.constant_socre.filter.and', req);
    }
    return andFilters || [];
  };
