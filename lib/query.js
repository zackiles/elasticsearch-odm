...
function QueryPromise(query, func){
  ...
  self.missing = function(value){
    addTermFilters(self.q, null, null, value);
    return self;
  };

  self.exists = function(value){
    addTermFilters(self.q, null, null, null, value);
    return self;
  };
...
function parseRequest(indices, types, matches, options){
  ...
  if(options.random){
    addQuery(req, {
      function_score: {
        random_score: { seed: Date.now().toString() }
      }
    });
  }

  if(options.must || options.not || options.missing || options.exists){
...
function addTermFilters(req, mustFilters, notFilters, missingFilters, existsFilters){
  var filter = {
    bool: {
      must: [],
      must_not: []
  ...
  var makeMissingFilter = function(v){
    return {
      missing: {
        field: v
      }
  ...
  var makeExistsFilter = function(v){
    return {
      exists: {
        field: v
      }
  ...
  if(!_.isEmpty(missingFilters)){
    missingFilters = _.isArray(missingFilters) ? missingFilters : [missingFilters];
    missingFilters = _.map(missingFilters, makeMissingFilter);
    filter.bool.must = filter.bool.must.concat(missingFilters);
  }
  if(!_.isEmpty(existsFilters)){
    existsFilters = _.isArray(existsFilters) ? existsFilters : [existsFilters];
    existsFilters = _.map(existsFilters, makeExistsFilter);
    filter.bool.must = filter.bool.must.concat(existsFilters);
  }
  ...
  var filterQuery = dot.pick('body.query.filtered.filter.bool', req);

  if(filterQuery){
    _.merge(req.body.query.filtered.filter.bool, filter.bool);
  }else{
    req.body.query = {
      filtered: {
...
function addQuery(req, query){
  ...
  if(dot.pick('body.query.filtered.query', req)){
    _.merge(req.body.filtered.query, query);
    if(req.body.filtered.query.match) delete req.body.filtered.query.match_all;
  }else{
    req.body.query = req.body.query || {};
    _.merge(req.body.query, query);
    if(req.body.query.match) delete req.body.query.match_all;
  }
}
