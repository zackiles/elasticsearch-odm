...
function addTermQuery(req, mustFilters, notFilters) {
  ...
  _.each(query.bool, function (v, k) {
    if (!v || !v.length) delete query.bool[k];
  });

  // No filters found? return now.
  if (_.isEmpty(query.bool)) return void 0;

  // Is this a new filtered query, or an existing to merge with?
  var boolFilter = dot.pick('body.query.bool', req);
  ...
  if (boolFilter) {
    if (query.bool.must) mergeFilter('must', query.bool.must);
    if (query.bool.must_not) mergeFilter('must_not', query.bool.must_not);
  } else {
    req.body.query = {
      bool: query.bool
      /*filtered: {
