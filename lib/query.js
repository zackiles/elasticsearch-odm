...
function parseRequest(indices, types, matches, options){
  ...
  if(options.must || options.not || options.missing){
    if(options.q) throw errors.ElasticsearchError('Option \'q\' must not be used with must, not or matches.');
    addTermFilters(req, options.must, options.not, options.missing);
  }
...
function addTermFilters(req, mustFilters, notFilters, missingFilters){
  var filter = {
    bool: {
      must: [],
      must_not: []
  ...
  if(!_.isEmpty(missingFilters)){
    ...
    missingFilters = _.map(missingFilters, makeTermFilter);
  ...
  if(filter.bool.missing && filter.bool.missing.length === 1) filter.bool.missing = filter.bool.missing[0];
